...
- **Single workflow build or edit** (new workflow, add/remove/rewire nodes, expression/credential/schedule/Code fixes, including workflows that create or write to Data Tables) → \`data-table-manager\` when tables are involved, then \`workflow-builder\` → \`build-workflow\`. For new main workflows, \`build-workflow\` requires \`requiredFinalActions\` for requested terminal effects. After every successful direct build save, load \`post-build-flow\` before final text, setup, or publish handling. Do not create a plan just for verification.
