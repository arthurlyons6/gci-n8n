...
describe('UserProxyLlm.respondToConfirmation', () => {
	...
	it('drops malformed setup-wizard node parameter entries', async () => {
		const agent = new FakeAgent();
		agent.enqueue({
			action: 'apply_setup_wizard',
			nodeParametersJson: JSON.stringify({
				'Send Slack Message': '#general',
				'Send Email': { to: 'ops@example.com' },
			}),
		});
		const proxy = new UserProxyLlm({
			conversation: [{ role: 'user', text: 'post and email the update' }],
			agent,
		});

		const response = await proxy.respondToConfirmation(setupWizardEvent('req-sw-malformed'));
		expect(response.kind).toBe('setupWorkflowApply');
		if (response.kind === 'setupWorkflowApply') {
			expect(response.nodeParameters).toEqual({
				'Send Email': { to: 'ops@example.com' },
